...
create_backup() {
    ...
    if [[ -f "/etc/sonic/custom-fan/fancontrol" ]]; then
        ...
        # platform default as fallback snapshot
        if [[ -d "/usr/share/sonic/device/$platform" && -f "/usr/share/sonic/device/$platform/fancontrol" ]]; then
            mkdir -p "$work/data/usr/share/sonic/device/$platform"
            cp -a "/usr/share/sonic/device/$platform/fancontrol" "$work/data/usr/share/sonic/device/$platform/fancontrol" || true
        fi
    fi
